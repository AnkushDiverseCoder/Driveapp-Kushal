// AdminDashboard.js (Optimized version)
...
export default function AdminDashboard() {
  ...
  const exportDailyEntry = async () => {
    ...
    try {
      ...
      // Fetch based on filters directly
      if (emails.length && s && e) {
        ...
        // fallback → fetch all, only when export pressed
        data = await dailyEntryFormService.listDailyEntry().then(r => r.data.documents || []);
      }

      const cols = Object.keys(data[0] || {}).map(k => ({ key: k, label: k }));
      await exportToExcel(data, 'daily-entry', cols);
    } catch (e) {
  ...
  const exportTripDetails = async () => {
    ...
    try {
      ...
      if (emails.length && s && e) {
        ...
        data = await tripService.listTrips().then(r => r.data.documents || []);
      }

      // fetch usernames for unique emails
